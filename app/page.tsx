...
export default function HomePage() {
  ...
  return (
    <div className="flex h-screen w-full flex-col max-w-md mx-auto bg-luxury-black shadow-2xl overflow-hidden">
      ...
      {/* Categories - Slide */}
      <div className="w-full overflow-x-auto no-scrollbar py-4 scroll-smooth snap-x snap-mandatory">
        <div className="flex gap-3 px-6">
          {categories.map((category) => (
            <button
              key={category.id}
              onClick={() => setSelectedCategory(category.id)}
              className={`flex flex-col items-center justify-center gap-1.5 px-4 py-3 rounded-xl shrink-0 transition-all min-w-[85px] snap-start ${
